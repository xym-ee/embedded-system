...
除了处理器本身，外设寄存器，也可以包含进去，此外物理世界也可以作为状态。
...
数字电路都带有 reset 的。即上电后的第一个状态。

初始状态决定了我们是否可以把自己的程序放在机器上执行。老的电脑的热启动，单独的一个键，就叫 reset。
