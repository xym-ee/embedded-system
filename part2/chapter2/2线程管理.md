...
`rt_thread_t` 为用于描述线程的结构体的指针的类型别名为，这也很合理。线程控制块里有优先级、线程名称、线程状态等，也包含线程与线程之间连接用的链表结构，线程等待事件集合等。
...
动态创建和静态创建的函数名很有意思，细微差别。体会动态返回地址和静态传入线程块地址的小区别。
...
新创建的线程为初始化态，还没到就绪线程的调度队列，还需要使用下面函数让线程进入调度态：
